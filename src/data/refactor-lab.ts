export type RefactorLabChoiceKey = "widget" | "function" | "leave";

export interface RefactorLabChoiceLabel {
  title: string;
  body: string;
}

export interface RefactorLabClassificationQuestion {
  prompt: string;
  snippet: string;
...
export interface RefactorLabOrderingProblem {
  title: string;
  prompt: string;
  success: string;
  retry: string;
...
export interface RefactorLabRetrievalQuestion {
  id: string;
  prompt: string;
  answer: RefactorLabChoiceKey;
  explanation: string;
...
export const refactorLabStages = [
  { key: "example", title: "예시 보기" },
  { key: "classification", title: "분류하기" },
  { key: "ordering", title: "순서 맞추기" },
  { key: "completion", title: "빈칸 채우기" },
  { key: "retrieval", title: "회상 퀴즈" },
] as const;

export type RefactorLabStageKey = (typeof refactorLabStages)[number]["key"];

export const refactorLabChoiceLabels: Record<RefactorLabChoiceKey, RefactorLabChoiceLabel> = {
  widget: {
    title: "위젯화",
...
export const refactorLabClassificationQuestions: RefactorLabClassificationQuestion[] = [
  {
    prompt:
      "아래 코드는 같은 카드 모양을 여러 화면에서 반복해서 쓰는 상황입니다. 가장 먼저 고려할 분리는 무엇일까요?",
    snippet: `Column(
  children: [
    Container(
  ...
  {
    prompt:
      "한 화면에만 쓰이는 짧은 제목 한 줄입니다. 지금 이 시점에서 가장 좋은 선택은 무엇일까요?",
    snippet: `const Text(
  "오늘의 할 일",
...
return null;`,
  ...
  {
    prompt:
      "온보딩에서 같은 모양의 단계 안내 박스가 아이콘과 문구만 바뀌며 네 번 반복됩니다. 어떤 분리가 더 적절할까요?",
    snippet: `Column(
  children: [
    Container(
      padding: const EdgeInsets.all(16),
  ...
  {
    prompt:
      "나이를 받아서 '미입력', '7세', '성인' 같은 문자열로 바꾸는 규칙을 여러 카드에서 씁니다. 어떤 분리가 더 적절할까요?",
  ...
  {
    prompt:
      "설정 화면에서 한 번만 쓰는 Divider 한 줄이 있습니다. 이 시점의 가장 좋은 선택은 무엇일까요?",
  ...
  {
    prompt:
      "저장 버튼과 자동 저장 흐름 모두에서 같은 submitProfile() 로직을 호출해야 합니다. 어떤 분리가 더 적절할까요?",
...
export const refactorLabOrderingProblems: RefactorLabOrderingProblem[] = [
  {
    title: "반복 UI를 새 위젯으로 빼는 순서",
    prompt:
      "결과 카드가 세 번 반복됩니다. 이 반복 UI를 StatCard 위젯으로 뺀다고 가정하고, 아래 단계를 올바른 순서로 맞춰보세요.",
    success:
  ...
  },
  {
    ...
    prompt:
      "이름 입력 검증이 제출 버튼과 자동 검증 둘 다에서 반복됩니다. 이를 validateName 함수로 뺀다고 가정하고 순서를 맞춰보세요.",
    success:
...
export const refactorLabRetrievalQuestions: RefactorLabRetrievalQuestion[] = [
  {
    id: "r1",
